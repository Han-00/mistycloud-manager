...
### 新增
- **换号事件流（events.jsonl）**：新增 `events.py`，换号 / 注册 / 回退 / 自愈在**结果确定处**各记一行结构化事件（时间 / 类型 / 账号 / 结果 / 触发原因 / 出口 IP / 耗时）。append-only JSONL + 超 512KB 自动裁剪至最近 500 条（临时文件 + `os.replace` 原子替换），线程安全、异常全吞——留痕永不影响业务。日志页新增「运行日志 / 事件」双标签，事件视图渲染为时间线（类型徽标按类型着色、失败标红）。埋点原则：只记结果不记中途，否则一次失败会留下多条自相矛盾的记录。
- **链路可用率（近 24h）**：`stats.py` 新增 `probe` 小时桶与 `record_probe()` / `availability(24)`。**分母是探测次数而非小时数**——监控没在跑的时段不该被当成「不可用」，否则关一晚上程序第二天可用率凭空下跌；`pct=None` 表示无数据、**区别于 0%**（「没测过」和「全挂」是两件事，新装用户不该一开机就看到红色 0%）。仪表盘新增「链路可用率」卡片（数字 + 水平进度条，≥95% 绿 / 90~95% 黄 / <90% 红，无数据显示「数据积累中」）。`monitor` 只在**真的探测过**之后才计数——因换号进行中跳过探测的轮次不计入。
...
### 调整
- **流量热力图改为固定档界 4 档离散色阶**：原先格子是主题 accent 单色的连续透明度渐变，相邻强度差异太小、一眼看不出层次。改为**用户拍板的固定绝对档界**（每小时消耗）：<50 MB 蓝 / 50~200 MB 黄 / 200~500 MB 橙 / ≥500 MB 红。固定档界跨周期可比——颜色不变就是用量没变；色板定义为 CSS 变量 `--heat-1..4`，格子着色与图例色块同源、不随换肤漂移，图例直接标注「<50MB / 50+ / 200+ / 500+」。

### 测试
- `tests/test_stats.py` 扩至 [20] 组：新增可用率五组（分母=探测次数、无数据 pct=None、24h 窗口外不计入、时钟回拨的未来桶不计入、坏键/非 dict 桶跳过）与 probe 修剪、**多实例隔离**。隔离这条是 [15] 顺带撞出来的真 bug（见修复节）。
- 新增 `tests/test_events.py`（11 组）：record/recent 往返与倒序、空字段不落盘、坏行容错、kind 过滤、limit 边界与非法值、文件不存在、超阈值裁剪（新事件必须保留、临时文件必须消失）、**8 线程并发写入 480 条无丢失无撕裂**、目录不存在/路径非法静默吞掉、clear 后仍可记录。
- `tests/test_ui_web_state.py` 扩至 13 组：`link_uptime` 透传与 stats 异常退化（不得中断整轮快照）、`get_events`/`clear_events` 转发与异常兜底。
- 前端布线静态校验：JS 引用的全部 65 个 DOM id 在 HTML 中逐一核对存在（抓「按钮点了没反应」类低级错误）。
...
- **`Stats` 多实例共享嵌套字典（潜伏 bug，测试顺带撞出）**：`__init__` 的 `dict(_DEFAULT)` 是浅拷贝，`hourly` / `probe` 这几个 `{}` 是**所有实例共享的同一个对象**——第二个实例往里写一桶，第一个实例立刻「看见」，保存时还会把对方的数据写进自己的文件。`traffic_base` 当初被单独补过一句 `= {}`，就是踩过这个坑但只修了一个键。生产只有一个单例所以从不露头；现改为逐键复制嵌套字典，并补指名道姓的隔离用例。
