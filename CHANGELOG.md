...
- **Web 界面补齐 5 项功能缺口**（玻璃版有、Web 版没有，属日常使用中的真实退化）：
  - **开机自启开关**（设置页）：走 `HKCU\...\Run`，在「任务管理器 → 启动」里可见、可关，不需要管理员权限。
  - **账号行右键菜单**：复制邮箱 / 查看详情 / 删除。菜单项按账号状态**动态生成**——在用号不出现「切换」与「删除」（其淘汰由换号流程接管），失效号不出现「切换」。
  - **快捷键**：`F5` 刷新流量、`Ctrl+T` 立即换号、`Esc` 收起弹窗与右键菜单。焦点在输入框内时不触发，避免填设置时误换号。
  - **操作结果浮窗（toast）**：右下角堆叠提示，成功绿边 / 失败红边，自动消隐且不拦截点击。换号、切换、删除、清理、保存设置、自启开关均接入——过去这些操作只有日志里能看到结果。
  - **日志清空按钮**：日志页与底部抽屉各一个，清空后抽屉红点提示同步复位。
...
- **浅色模式下日志区几乎不可读**（`webui/app.css`）：`.logpage` 与 `.drawer-body` 用的是硬编码深色底，前景色却继承主题变量——浅色模式下变成「深灰字压近黑底」，实测对比度仅 **1.25**（WCAG AA 正文要求 4.5），抽屉标题 3.92 亦不达标。此前一直没暴露，只因日志行大多带 `err`/`ok` 颜色类，只有纯文本行才会撞上。现将「终端表面」（日志页 / 抽屉）固定为深底浅字、**不随深浅模式切换**——终端本就该是深色，且日志的语义色是按深底调过的，跟随模式反而集体失色；抽屉标题另立 `--con-*` 变量组，对比度 3.92 → **6.33**，日志正文 1.25 → **16.52**。
- 清理结果的提示语义：无失效账号可清理时原本弹红色 ✗，读起来像失败，实际是操作成功——改为中性提示，只有真的失败才用红色。
...
- 新增 `tests/test_ui_web_state.py`（8 组）：`build_state` 字段齐全、账号行 `valid`/`remain_s`/`remain_pct` 计算、无在用账号、流量余量与低量标记、续航预测、查询失败分支，以及**删除保护规则**（在用号拒绝删除、换号进行中拒绝删除与清理、批量清理只动 expired/banned）。上述 KeyError 正是它第一次运行抓出来的。后扩展至 **11 组**，补上 toast 入队与成功/失败标记、`get_settings` 的 autostart 字段与模块同源、`set_autostart` 成功与受限两条路径。
- **开机自启抽为 `autostart.py`（全项目唯一实现）**：该功能原先只写在 `ui_glass.py` 内，Web 版补齐时若直接照抄就成了第二份复制——正是 `settings_schema` 当年「同一件事两处实现、改一处漏一处」的翻版。现玻璃版与 Web 版共用这一份，模块边界严格：**只读写注册表，不落盘、不打日志、不弹提示**（三个界面的呈现方式各不相同），失败抛 `OSError` 交由调用方决定怎么呈现。
- 新增 `tests/test_autostart.py`（9 组）：`_command()` 的打包态 / 源码态 / `pythonw.exe` 缺失兜底三条路径；`is_enabled()` 对「未设置 / 注册表不可读 / 无 winreg」一律返回 `False` 而不抛——设置页加载时要读它，抛异常会连累整个设置页打不开；`set_enabled()` 开→读得到、关→读不到、重复关幂等、写不进去时抛 `OSError`（**不能静默成功**，否则用户会以为设上了）。**全程用内存版 winreg 顶掉真实注册表**——往 `HKCU\...\Run` 写测试值等于在用户机器上留下一个指向测试目录的自启项，测试结束也不会自己消失。
